...
- `GET https://api.imdbapi.dev/names/{nameId}` - Get detailed actor information
- `GET https://api.imdbapi.dev/names/{nameId}/filmography?categories=ACTOR&categories=ACTRESS&pageSize=50` - Get actor filmography
- `GET https://api.imdbapi.dev/titles/{titleId}` - Get movie details
- `GET https://api.imdbapi.dev/titles/{titleId}/credits?pageSize=50` - Get movie cast and crew
- `GET https://api.imdbapi.dev/search/titles?query={query}&limit=10` - Search for movies/shows
...
## IMDb API Migration Notes

As of September 2025, imdbapi.dev has migrated from `rest.imdbapi.dev` to `api.imdbapi.dev` with updated endpoints and parameter names.

Key changes applied in this project:
- Base URL updated to `https://api.imdbapi.dev`
- Replaced `/names/{id}/known_for` with `/names/{id}/filmography` (supports `categories` and `pageSize`)
- Query parameters changed to camelCase (e.g., `pageSize`, `limit`, `query`)
- Search endpoint now uses `/search/titles?query=...`
- Title runtime is provided in `runtimeSeconds` and is converted to minutes in the UI

No API key is required; the service remains free to use. If you previously forked or integrated this project, ensure your local builds use the updated endpoints.
